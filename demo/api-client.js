...
export async function fetchDepartures(stopId, queryTime = null) {
    try {
        const xmlBody = createOJPRequest(stopId, queryTime);
...
function deduplicateDepartures(departures) {
    ...
    for (let i = 0; i < departures.length; i++) {
        ...
        const key = `${dep.journeyNumber}|${dep.destination}`;
...
function parseOJPResponse(xmlString) {
    ...
    function getStopRef(callNode) {
        const parentRef = callNode.getElementsByTagName('ParentRef')[0]?.textContent?.trim();
        if (parentRef && /^\d{7}$/.test(parentRef)) return parentRef;
